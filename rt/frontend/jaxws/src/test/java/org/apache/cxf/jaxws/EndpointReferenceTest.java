...
public class EndpointReferenceTest extends AbstractJaxWsTest {
	...
	public void testBindingProviderSOAPBinding() throws Exception {
		javax.xml.ws.Service s =
				javax.xml.ws.Service.create(new QName("http://apache.org/hello_world_soap_http", "SoapPort"));
		assertNotNull(s);
...
//		org.apache.hello_world_xml_http.bare.XMLService s = new org.apache.hello_world_xml_http.bare.XMLService();
//
...
//		}
	...
	/*
	 * Any JAX-WS supported epr metadata MUST match the Service instances ServiceName, otherwise a WebServiceExeption
	 * MUST be thrown. Any JAX-WS supported epr metadata MUST match the PortName for the sei, otherwise a
	 * WebServiceException MUST be thrown. If the Service instance has an associated WSDL, its WSDL MUST be used to
	 * determine any binding information, anyWSDL in a JAX-WS suppported epr metadata MUST be ignored. If the Service
	 * instance does not have a WSDL, then any WSDL inlined in the JAX-WS supported metadata of the epr MUST be used to
	 * determine binding information. If there is not enough metadata in the Service instance or in the epr metadata to
	 * determine a port, then a WebServiceException MUST be thrown.
	 */
	@Test
	...
	public void testEndpointGetEndpointReferenceSOAPBinding() throws Exception {
		...
		Element referenceParameters =
				fetchElementByNameAttribute(doc.getDocumentElement(), "wsa:ReferenceParameters", "");
	...
	public void testEndpointGetEndpointReferenceW3C() throws Exception {
		...
		EndpointReference endpointReference =
				endpoint.getEndpointReference(W3CEndpointReference.class, referenceParameters);
		assertNotNull(endpointReference);
	...
	public void testProviderCreateW3CEndpointReference() throws Exception {
		...
		W3CEndpointReference endpointReference = provider.createW3CEndpointReference("http://localhost:8080/test",
				serviceName, portName, metadataList, "wsdlDocumentLocation", referenceParameters);
		assertNotNull(endpointReference);
	}
